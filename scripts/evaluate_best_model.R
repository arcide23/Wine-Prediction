...
if (best_model == "intercept") {
  ...
  alpha <- if (grepl("^lasso", best_model)) 1 else 0
  res <- fit_predict_glmnet_multinom(x_train, train_df$quality, x_test, alpha, fold_seed_offset = 900)
  pred <- res$pred
  tuned_lambda <- res$lambda
} else if (best_model %in% c("random_forest", "random_forest_all_plus_squared")) {
  if (!requireNamespace("randomForest", quietly = TRUE)) {
    stop(
      "Best model is '", best_model, "' but package 'randomForest' is not installed.\n",
      "Install it with:\n",
  ...
  if (best_model == "random_forest_all_plus_squared") {
  ...
  stop("Unknown best model name: ", best_model)
}

final_acc <- get_accuracy(test_df$quality, pred)
cat("Best model (by CV avg_accuracy):", best_model, "\n")
if (!is.na(tuned_lambda)) cat("Tuned lambda (train CV):", tuned_lambda, "\n")
cat("Final test accuracy:", final_acc, "\n")
